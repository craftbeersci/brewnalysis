...
fermentation_data <- read_excel("~/brewnalysis/brewnalysis/data/ccb_ale1_tidy_fermdata.xlsx") %>%
  janitor::clean_names() %>% na.omit() %>%
  dplyr::select(brand, batch_num,ferm_day,sample_time,cell_count) 
...
fermentation_data <- dplyr::mutate(fermentation_data,cell_density=cell_count*10^6)
#______________________________________________________________________________________________________
