...
```{r}
data <- read_excel("data/brewnalysis_test_data.xlsx") %>%
  janitor::clean_names() %>%
...
data1 <- sapply(data,cell_count_func,cell_count_1_squares_counted,cell_count_1_live_cells,dilution_factor)

#data2 <- sapply(data,TvecFuncATB)

#celldenstime=data.frame(data2,data1)


#Tvec<-c(celldenstime)
#cell.density<-c(data1)
#DataFinal<-data.frame(Tvec,cell.density)


plot<- ggplot(data=data1,aes(x=Tvec,y=cell.density))+geom_point(aes(colour = factor(Condition)))+geom_line(aes(colour = factor(Condition)))+labs(title = "Cell density in wort according to fermentation time", x="Fermentation time (hours)",y="Cell density")
plot
...
# Alternative Visualization of all the batches
