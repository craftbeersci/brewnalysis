...
exp_guess <- c(max(obs), .5, min(obs))
exp_fit2 <- optim(par = exp_guess[1:2], fn = exp_dev_sq2, method = "Nelder-Mead", fg = min(obs), tvec = tvec, obs = obs)
...
exp_pred2 <- exp_func(c(exp_fit2$par, min(obs)), tvec)
...
model_fit_plot <- ggplot(beer_a_b5, aes(x = fermentation_day, y = gravity)) +
  ...
  geom_point(aes(x=lineVec, y = exp_pred2), color = "green", show.legend = TRUE) +
  geom_line(aes(x=lineVec, y = exp_pred2), color = "green") +
  theme_classic()
...
exp_summary_2 <- function(exp_guess, tvec, obs){
  exp_pars <- optim(par = exp_guess[1:2], fn = exp_dev_sq2, method = "Nelder-Mead", fg = min(obs), tvec = tvec, obs = obs)
  exp_pars <- exp_pars$par
...
model_fits <- fermentation_data %>%
  ...
  summarize(log_pars = log_summary(log_guess, fermentation_day, gravity),
            ...
            exp_pars2 = exp_summary_2(exp_guess, fermentation_day, gravity)) %>%
  ...
  mutate(exp_pred_2 = exp_func(unlist(exp_pars2), fermentation_day)) %>%
  pivot_longer(c(gravity, log_pred, exp_pred, exp_pred_2))
