...
exp_guess <- c(max(obs), .5, min(obs))
exp_fit2 <- optim(par = exp_guess, fn = exp_dev_sq2, method = "Nelder-Mead", fg = min(obs), tvec = tvec, obs = obs)
...
model_fits <- fermentation_data %>%
  ...
  pivot_longer(c(gravity, log_pred, exp_pred, exp_pred_2), names_to = "model_id", values_to = "specific_gravity")
